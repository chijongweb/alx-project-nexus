...
class Product(models.Model):
    ...
    product_price = models.DecimalField(max_digits=10, decimal_places=2, db_index=True)  
    product_stock = models.IntegerField()
    product_image_url = models.URLField(blank=True)
    is_product_active = models.BooleanField(default=True)
    category = models.ForeignKey(Category, on_delete=models.CASCADE, related_name='products', db_index=True)  # Added db_index=True
